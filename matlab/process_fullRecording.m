        ...
        if SORTER_PATH
            ...
            for probe = 1:numel(metadata.probe_type)
                ...
                if isequal(metadata.probe_type{probe},'plexon')
                    ripple_info = loadMetadataJSON(fullfile(session_path,'ripple_info.json'));
                ...
                elseif isequal(metadata.probe_type{probe},'neuropixel')
                    ...
                    % Pull out SpikeInterface sorting outputs
                    [spikes_perTrial,sorting,trlAvg_frs] = parse_SortingToTbl(tbl,fullfile(si_path,'sorter_output'), 'NP_ALIGN_PULSES', these_alignTimes, 'Fs', ap_meta.imSampRate);
                ...
                if nevnum==1
                    ...
                    if isfile(fullfile(si_path,'quality_metrics','cluster_metrics.csv'))
                        ...
                        sorting.clusters.sess_name = repmat(S1.sess_name,height(sorting.clusters),1);
                        sorting.clusters.probe_id = repmat(S1.metadata.probe_index(probe)-1,height(sorting.clusters),1);
                        sorting.clusters.probe_label = repmat(S1.metadata.probe_label{probe},height(sorting.clusters),1);
                        sorting.clusters.probe_type = repmat(S1.metadata.probe_type{probe},height(sorting.clusters),1);
                        sorting.clusters.probe_config = repmat(S1.metadata.probe_config{probe},height(sorting.clusters),1);
                        sorting.clusters.hardware_config = repmat(S1.metadata.hardware_config{probe},height(sorting.clusters),1);
                        sorting.clusters.probe_depth_mm = repmat(S1.metadata.probe_depth_mm(probe),height(sorting.clusters),1);
